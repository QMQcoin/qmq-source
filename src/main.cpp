...
#if defined(NDEBUG)
# error "qmq cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4f8736d5221d0b03ed21145c647fd8431299c4242163cc2eba023ad186937c41");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // qmq: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "qmq Signed Message:\n";
...
{
    // qmq: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // qmq: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // qmq: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1468512306, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "dsdasd";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04953f7f311ca41b6d3711b852a4c4902cc3727bc9449d33671ad482a92da1d4a564ac03ef6a530be5fe2c5b06e2147c425b0938e91b80290e27eb3276c7b2e577") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1468512306;
        ...
        assert(block.hashMerkleRoot == uint256("0x183d519b9fb476749142241f09ca211b79ffc7e40c654543ce6636e3fa6f684a"));
...
unsigned char pchMessageStart[4] = { 0x23, 0xaf, 0x95, 0xc5 }; // qmq: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("qmqMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("qmqMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static qmqMiner(CWallet *pwallet)
{
    printf("qmqMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("qmq-miner");
    ...
    try { loop {
        ...
        printf("Running qmqMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&qmqMiner, pwallet));
}
